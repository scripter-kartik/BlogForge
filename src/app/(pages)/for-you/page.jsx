...
export default function ForYouPage() {
  ...
  const renderPosts = (list) => {
    ...
    return list.map((post) => (
      ...
      >
        <div
          className={`flex flex-col md:flex-row gap-4 md:gap-6 p-4 md:p-6 rounded-xl transition-all duration-300 hover:shadow-xl hover:scale-[1.01] ${
          ...
          <div className="flex flex-col gap-3 sm:gap-4 flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {post.authorUsername ? (
                  <Link
                    ...
                    className={`text-sm font-medium truncate hover:underline ${isDarkMode ? "text-white" : "text-black"}`}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {post.tags?.slice(0, 3).map((tag, idx) => (
                <div
                  key={idx}
                  className={`px-2 sm:px-3 py-1 text-xs font-medium rounded-full transition-colors duration-300 ${
